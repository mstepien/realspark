...
def fast_analysis(monkeypatch):
    ...
    def mock_prepare(content):
        ...
        w, h = img.size
        return img, np.zeros((h, w, 3)), w, h, np.array([0, 0, 0])
...
@pytest.mark.anyio
async def test_get_stats_empty(mock_db_connection):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
...
@pytest.mark.anyio
async def test_upload_image_flow(mock_db_connection, mock_storage_client):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
...
@pytest.mark.anyio
async def test_partial_results_flow(mock_db_connection, mock_storage_client):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
...
@pytest.mark.anyio
async def test_task_abandonment_logic(mock_db_connection, mock_storage_client):
    transport = ASGITransport(app=app)
